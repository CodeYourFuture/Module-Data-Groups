...
window.addEventListener("load", () => {
  ...
  document
    ...
    .addEventListener("click", deleteCompletedTasks); //added
  // delete task button for delete completed task. when the user clicks this button, deleteCompletedTasks
  //function called
...
// Handles the Delete Completed Tasks button click.
function deleteCompletedTasks() {
  Todos.deleteCompleted(todos); // deleteCompletedTasks function runs and call Todos.deleteCompleted(todos).
  //then Todos.deleteCompleted(todos) go to .mjs file and removes all completed tasks from the array.
  render(); //rebuilds the HTML list using the updated todos array.
}
