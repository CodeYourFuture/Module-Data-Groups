...
if (!Array.isArray(pairs)) {// check if the input is an array. If not, throw an error.
...
for (const pair of pairs) {// iterate through each pair in the input array
    if (!Array.isArray(pair) || pair.length !== 2) {// check if the current pair is an array of length 2. If not, throw an error.
