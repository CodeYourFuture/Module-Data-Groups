...
// 〰️ Step 2: Read the content of the input.txt file.
...
fs.readFile('./Sprint-1/stretch/aoc-2018-day1/input.txt', 'utf8', function (error, data) {
  // 〰️ Step 3: If there's an error (e.g., the file doesn't exist), log it and stop the program.
  if (error) {
    console.log('Error reading the file:', error);
    return;
  }

  // 〰️ Step 4: Split the file content into an array of strings, where each string is a line from the file.
  // 〰️ - The file has one frequency change per line, so we use '\n' (newline) to separate them.
  const changes = data.split('\n'); 

  // 〰️ Step 5: Initialize the starting frequency to 0.
  let frequency = 0; 

  // 〰️ Step 6: Loop through the array of changes and add each value to the frequency.
  ...
  for (let i = 0; i < changes.length; i++) {
   ...
   // 〰️ Step 7: Print the final calculated frequency to the console.
