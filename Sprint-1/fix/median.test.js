...
const { calculateMedian, calculateMedianImproved } = require("./median.js");

if (calculateMedianImproved) {
  describe("calculateMedianImproved", () => {
    [
      ...
      it(`returns the median for [${input}]`, () => expect(calculateMedianImproved(input)).toEqual(expected))
    );
    ...
    [
      ...
      it(`returns the correct median for unsorted array [${input}]`, () => expect(calculateMedianImproved(input)).toEqual(expected))
    );

    it("doesn't modify the input array [1, 2, 3]", () => {
      const list = [1, 2, 3];
      calculateMedianImproved(list);
      expect(list).toEqual([1, 2, 3]);
    });

    [ 'not an array', 123, null, undefined, {}, [], ["apple", null, undefined] ].forEach(val =>
      it(`throws an error for non-numeric array (${val})`, () => expect(() => calculateMedianImproved(val)).toThrow())
    );
    ...
    [
      ...
      it(`filters out non-numeric values and calculates the median for [${input}]`, () => expect(calculateMedianImproved(input)).toEqual(expected))
    );
  });
}
