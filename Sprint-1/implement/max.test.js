...
describe("findMax", () => {
  ...
  test("given an array with both positive and negative numbers, returns the largest number", () => {
    expect(findMax([-10, 0, 50, 20, -30])).toEqual(50);
  });

  test("given an array with just negative numbers, returns the closest one to zero", () => {
    expect(findMax([-100, -50, -5, -1])).toEqual(-1);
  });

  test("given an array with decimal numbers, returns the largest decimal number", () => {
    expect(findMax([1.5, 2.8, 3.3, 0.1])).toEqual(3.3);
  });

  test("given an array with non-number values, returns the max ignoring non-numeric values", () => {
    expect(findMax(["hello", 10, "world", 60, 20])).toEqual(60);
  });
