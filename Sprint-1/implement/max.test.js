...
test("given an empty array, returns -Infinity", () => {
  expect(findMax([])).toBe(-Infinity); // we implented a call back function to check if the function returns -Infinity when the array is empty
});
...
test("given an array with decimal numbers, returns the largest decimal number", () => {
    expect(findMax([1.1, 1.2, 1.3])).toBe(1.3);
}); 
...
expect(findMax(["hey", 10, "hi", 60])).toBe(60);
});
