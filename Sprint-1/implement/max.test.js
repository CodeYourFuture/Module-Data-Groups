...
test("given an array with positive and negative numbers, returns the largest number", () => {
    expect(findMax([-1, 5, -10, 3])).toBe(5);
});
...
test("given an array with decimal numbers, returns the largest decimal", () => {
    expect(findMax([1.5, 2.7, 0.9, 3.1])).toBe(3.1);
});

// Given an array with non-number values
// When passed to the max function
// Then it should return the max and ignore non-numeric values
test("given an array with non-number values, returns max of numeric values", () => {
    expect(findMax(['hey', 10, 'hi', 60, 'test'])).toBe(60);
});
...
test("given an array with only non-number values, returns -Infinity", () => {
    expect(findMax(['hey', 'hi', 'test'])).toBe(-Infinity);
});
